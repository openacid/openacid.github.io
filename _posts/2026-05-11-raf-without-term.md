...
*Standard Raft persists a separate `current_term` and an array of `(term, command)` log entries. `raf` is similar, but splits the term and command at each index into two aligned `Vec`s.*
...
![Leader election term=4](/post-res/raf-without-term/289a45a69ad1dd8d-leader-election-term4.png)
...
![RequestVote scenarios](/post-res/raf-without-term/d310c7c93bee8cbb-request-vote.png)
...
1.  Check the follower's local log at `prev_log_id`. If it does not match, return the conflict index.
1.  If `prev_log_id` matches, process the real entries starting at `prev_log_id.index + 1`.
1.  If later local commands diverge from the leader, truncate the local commands.
1.  Overwrite the local `terms` range covered by this request.
1.  Append only the commands that are missing locally.
