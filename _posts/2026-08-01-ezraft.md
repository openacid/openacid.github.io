...
![](/post-res/ezraft/f45129e0aa8db968-ezraft-banner.png)

# EzRaft: Build a Distributed KV Store in 100 Lines

For the past few years, I have spent most of my time working on [OpenRaft](https://github.com/databendlabs/openraft).

OpenRaft is an open-source project that runs in production at companies around the world. To support such a wide range of applications, it has to be highly flexible. Log entries, log IDs, state machines, and storage are all expressed as [Rust](https://www.rust-lang.org/) traits, so users can adapt them to their own needs.

Today, OpenRaft's API and performance are both fairly mature. The trade-off is that its model of the [Raft](https://raft.github.io/) algorithm remains quite abstract.
...
1.  Understanding OpenRaft's own abstractions:
...
I began to wonder: could I build a layer that lets people focus on their business logic first, without requiring them to master Raft, Rust, and OpenRaft up front?

That question led to EzRaft.

[EzRaft](https://github.com/drmingdrmer/ezraft) is a streamlined distributed storage library built on top of OpenRaft. It hides the details you do not need at the beginning, so you can concentrate on the application rather than the machinery underneath it.
...
![The layers of EzRaft and OpenRaft: users implement EzApp, EzRaft provides storage, networking, and service APIs, and OpenRaft handles consensus](/post-res/ezraft/db3f610ca0318cb2-ezraft-layers.png)
...
1.  The shape of a response: `Response`. Requests and responses represent your application's operations, so only you can define them.
1.  How a request changes the state machine: `apply()`.
1.  How to read from the state machine: `read()`. You can think of the state machine as a key-value store and use a key to retrieve one part of it.
...
1.  Assemble a service from the components EzRaft provides.
1.  Start three nodes and try the cluster for yourself.
...
The complete application state lives in a KV map, backed here by a [B-tree map](https://doc.rust-lang.org/std/collections/struct.BTreeMap.html).
...
To keep the example readable, I omitted the derive attributes. A real implementation needs them. `Request` travels over the network and is written to the Raft log, so it must support serialization through [serde](https://serde.rs/).
...
The repository includes a complete, runnable version in [examples/kvstore.rs](https://github.com/drmingdrmer/ezraft/blob/main/examples/kvstore.rs), with all derives and imports in place. You can run it directly with `cargo run`.
...
1.  A seed address. A new node with uninitialized storage contacts the seed to join an existing cluster.
...
![How three EzRaft nodes form a cluster: the first creates it, while two more join through the seed, catch up, and become voters](/post-res/ezraft/1b91902c8e63f4b5-ezraft-cluster-formation.png)
...
A production system should replace it with a proper storage backend. `FileStorage` does not call fsync, so a power failure could lose a write that the cluster has already acknowledged to the client. That can [break Raft's correctness guarantees](https://blog.openacid.com/algo/raft-io-order-complete-cn/).
...
This local read is fast, but it can be slightly stale. The node may not yet have received a write that the cluster just acknowledged. If you need a strictly [linearizable](https://blog.openacid.com/algo/linearizable/) read, call `linearizable()` first. It [confirms the leader's identity](https://blog.openacid.com/algo/openraft-read/) and waits for the local state to catch up; the following `read()` will then see every earlier acknowledged write.
...
The two remaining nodes still form a [majority](https://blog.openacid.com/algo/quorum/), so they elect a new leader within a few heartbeats. The data written before the failure remains available, and the cluster continues to accept writes:
...
![Failover in a three-node cluster: after the original leader fails, the other two nodes form a majority and elect a new leader](/post-res/ezraft/85aed0f352257dfd-ezraft-failover.png)
...
![How node IDs 0, 2, and 6 relate to Raft log indexes written by join, add learner, and membership change operations](/post-res/ezraft/57254dc9bd5dea86-ezraft-node-id-log.png)
...
The data is usually small, but it is critical: losing it or allowing replicas to disagree can disrupt every system built on top of it. [ZooKeeper](https://zookeeper.apache.org/) and [etcd](https://etcd.io/) serve this same class of workload.
...
That makes the example easy to experiment with. Stop a node and watch the cluster elect a new leader. Follow the log index in the metrics. Change the heartbeat interval in `EzConfig` and see how it affects election time. Once these ideas are concrete, moving down a layer to [OpenRaft](https://github.com/databendlabs/openraft) is much easier than starting with a set of trait definitions.
...
-   Code: [github.com/drmingdrmer/ezraft](https://github.com/drmingdrmer/ezraft)
...
- I/O Ordering in Raft : [https://blog.openacid.com/algo/raft-io-order-complete-cn/](https://blog.openacid.com/algo/raft-io-order-complete-cn/)
...
- ezraft kvstore example : [https://github.com/drmingdrmer/ezraft/blob/main/examples/kvstore.rs](https://github.com/drmingdrmer/ezraft/blob/main/examples/kvstore.rs)

- openraft : [https://github.com/databendlabs/openraft](https://github.com/databendlabs/openraft)

- BTreeMap : [https://doc.rust-lang.org/std/collections/struct.BTreeMap.html](https://doc.rust-lang.org/std/collections/struct.BTreeMap.html)
