...
![](/post-res/slimtrie-impl/f5b29d15d1490367-slim-2-01.webp)
...
![](/post-res/slimtrie-impl/cbff7aa94a492ea6-slim-2-02.webp)
...
![](/post-res/slimtrie-impl/4c42e0624ecdb37c-slim-2-03.webp)
...
![](/post-res/slimtrie-impl/b4aed5f8621f3760-slim-2-04.webp)
...
![](/post-res/slimtrie-impl/cbb3158ddf89987b-slim-2-05.webp)
...
![](/post-res/slimtrie-impl/6e72e9c3ef514c7e-slim-2-06.webp)

上图中可以看到:

1.  SlimTrie 作为索引在内存占用上显著低于 map 和 B-Tree。
1.  SlimTrie 作为索引其内存占用的决定因素是 key的数量，与key的大小无关。
...
![](/post-res/slimtrie-impl/97f427e674116dd2-slim-2-07.webp)
...
![](/post-res/slimtrie-impl/cfe153067d0fab83-slim-2-08.webp)

### 不存在的 key 的查询效率对比

越小越优:

![](/post-res/slimtrie-impl/6e93c458d075c61f-slim-2-09.webp)

可以看出 SlimTrie 的查询效率与 无序的hash类型的Map 相比有差距, 但对比同样是保持了顺序性的btree相比, 性能是Btree的 2.6倍 左右.

### 查询时间随k和n的变化

![](/post-res/slimtrie-impl/00fad3cec2efcbbc-slim-2-10.webp)

另一方面，在上图中，我们也能够看到，SlimTrie 的实际查询的耗时在 150ns 左右，加上优秀的空间占用优化，作为存储系统的索引依然有非常强的竞争力。

### 内存开销随k和n的变化

![](/post-res/slimtrie-impl/bb40bbde3fd0b903-slim-2-11.webp)
