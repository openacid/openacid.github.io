...
![](c915c4fcc98591ed-single-log-joint-banner.webp)
...
![图 1：Joint Consensus 两阶段流程](a7acea752fd84833-raft-joint.x.svg)
...
-   **初始状态**: 假设当前集群 config 为 `{a,b,c}`, 对应的 config log entry
    ...
    ![图 2：单条日志变更 - 初始状态](667db9f105260fea-single-1-start.x.svg)
...
-   **进入 Joint state (关键步骤)**: 当 Leader **看到** `entry-i` 时(即,
    ...
    ![图 3：单条日志变更 - 进入 Joint state](64bd23d7d6bd8fd7-single-2-joint.x.svg)
...
![图 4：补丁-1 持久层问题示例](3e6fb36ca8cf87de-single-3-elect.x.svg)
...
![图 5：补丁-2 新 Node 重启状态示例](004bf3df9c4de529-restart.x.svg)
...
![图 6：重启后回退到C-old-new](0507172bcfea4f35-restart-after-uniform.x.svg)
...
![图 7：补丁-3 引入 Barrier 条目流程](a1b3d0adfde6319d-barrier.x.svg)
...
![图 8：Barrier 条目重启后示意](431e950cac307092-barrier-restart.x.svg)
