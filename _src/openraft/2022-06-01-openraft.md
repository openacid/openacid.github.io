...
article:
    image: /post-res/openraft/openraft-47x10.jpg

mathjax: true
toc: true
toc_label: 本文目录
toc_sticky: true
excerpt: "Raft 做对了什么, 做错了什么, Openraft项目如何解决这些问题"
---
...
Raft 几乎成为分布式一致性协议的标准答案, 它提供了强一致, [CAP][] 中CA的分布式状态机设计.

虽然它的论文没有太多新意, 但它的价值在于将抽象的[paxos][] 跟工程实践之间的空隙填补了.

paxos与工程距离太远, 几乎无法直接使用. 即使比较贴近工程的[multi-paxos][], 也缺少了太多东西.



而multi-paxos没有, multi-paxos没有明确的是多个paxos-instance 与状态机的关系.

而Raft, 直接将答案给出, 它完整的定义了整个系统的状态机, 以及其运行中的每一步的因果链. 

工程师完全不用去理解分布式协议的原理, 只需按照spec编写代码, 就可以正确实现一个分布式强一致存储.

这是Raft最大的价值, 是它做对的事情.

而Raft为了讨好工程师, 在设计上也生硬的简化了一些概念, 使它**看起来**更简单, 代价是更难于理解和证明, 然而Raft的目标用户是工程师, 很多时候只关心最少加班几天能上线, 于是这些拙劣的设计虽然让人不舒服, 并带来了颇多问题, Raft还是最受欢迎的一份标准答案.

但实际上, 我们并不需要忍受这些掩耳盗铃的删减, 也可以有一个即便于理解也便于实现的分布式一致性框架.

![raft](/post-res/openraft/raft-overview.png)

# Raft 做对的事情

Raft 取巧的作法并不推崇, 但它在分布式一致性协议领域做对了2件事情, 将待解决的问题约束到一个清晰的框架里:

## Right: 单值系统

multi-paxos 为了在单值的paxos 协议上实现一个状态机的[WAL][] 日志, 将每条日志定义为一个paxos-instance.

但它并没有明确的定义每个日志,也就是paxos-instance之间的关系, 这部分完全需要工程师自己去思考和解决.

还在加班的工程师选择了Raft.

因为整个系统是一个整体, 因此也就可以将整个分布式状态机看做一个 **值**, 在[abstract-paxos][] 里, 我们描述了index(几乎相当于raft的term)与日志之间的关系, 解释了为何raft的term必须跟日志index一起决定leader等问题.

从而打通了paxos中单值的设计与现实中多值的日志的联系. multi-paxos中将多个paxos-instance组合在一起去描述日志是一个偷懒的思路, paxos的单值扩展为多个互相**兼容**的单值, 就可以直接得到raft的设计. 如[abastract-paxos][] 里的解释.

## Right: joint 成员变更

Raft中描述的joint 成员变更算法, 几乎接近理论最简洁的模型了. 它将成员变更问题抽象到一个足够小的范围, 解决了足够多的问题.

# Raft 做错的事情

## Wrong: Term

[abstract-paxos][] 中我们解释了为何必须引入index这个概念, 才能保证**commit**完成的状态可以被读到.

raft为了让问题看起来更简单, 只用一个整数term, 而不像paxos中那样要求round-number(rnd)是一个自增数字+proposer-id来确保唯一.

为了达到这个**简化** 模型的目的, raft必须在算法上解决类似paxos中rnd冲突的问题: 于是raft的phase-1, 也就是elect的阶段, 有了这样一个奇怪的设定:

在选举时, candidate A的term 如果不小于一个node的term, 且这个node没有投票给任何其他candidate, 才能投票给A.

在每个node上存储着它的term和voted_for:
...
V.term >= N.term && (N.voted_for.is_none() || N.voted_for == Some(candidate_id))

这让V的值形成了一个偏序关系, 即有2个V之间是不能比较大小的.

即V_1 {term:2, voted_for:Some(A)} 与 V_2 {term:2,voted_for: Some(B)} 是没有不能比较的.
...
一行 derive 就能解决的事情, 必须要手动实现并添加测试.

更严重的问题是, 一个值空间里有不能比较大小的值, 让选举成功的几率变低了.

也就是说, 它会引入更多的选举失败.

而在paxos中则没有这个问题, 一个节点如果选了proposer_1, 那么任何proposer_2 只要大于proposer_1,就可以覆盖之前的选票.

而Raft为了让proposer_id 从至少双值变成单值, 增加了实现复杂度 和 增加了在论文中不易验证的选举失败率.

要解决这个问题, 也很容易, 只需重新定义leader的标识:
...
同时,它也降低了选举的冲突, 一个term中, 如果有并发的3个node发起选举, 一定有一个node会成功, 而原始Raft中,有可能3个都失败.

这是 [openraft][] 对raft做的第一个优化.
...
- Databend: 
- TinyTrader: https://blog.csdn.net/weixin_41308834/article/details/124960923
-
