...
A timestamp oracle issues one globally increasing sequence. Transaction systems built on [MVCC][wiki-mvcc] consume two of those values on every commit, so the oracle sits on the critical path of every write. The naive way to keep the sequence safe is one Raft commit per request. That design is correct. It is also too slow.

This article builds the other design: reserve a range with one Raft write, then allocate from memory. The service is [timestamp_oracle][repo-ezraft-timestamp-oracle], about 200 lines on [EzRaft][repo-ezraft]. It runs as a three-node cluster, stays available after any one node fails, and answers `next_timestamp` without waiting for a Raft commit.

[EzRaft][repo-ezraft], from the [previous article][post-ezraft-cn], is a small [Rust][rust-lang] API over [OpenRaft][repo-openraft]. It hides the [Raft][raft-website], networking, and storage machinery, so you can start with the service itself.
...
![Architecture overview: `next_timestamp` requests allocate timestamps from the leader's memory, while the reservation task connects the in-memory cache to a three-node EzRaft cluster](assets/time-server-architecture.png)
...
Those values usually order distributed transactions. Models such as [Percolator][ref-percolator] give each transaction a start timestamp and a commit timestamp. Those two numbers decide which transaction came first. TiDB's [TSO (Timestamp Oracle)][doc-tidb-tso] is the same component.
...
2. Fault tolerance. The service stays available while a majority of the cluster is alive. OpenRaft already provides that, so a three-node cluster can lose any one node and keep issuing timestamps.

3. Low latency. `next_timestamp` sits on the critical path of every operation that needs an ordering point, so each request has to be cheap.
...
`Reserve` asks the state machine to move `reserved_end` to at least `reserve_upto_us`. The response is the newly claimed half-open interval `[start, end)`. Those types and `apply()` are all that [`EzApp`][docs-ezraft-ezapp] needs. See [timestamp_oracle.rs:66-112][code-ezapp].
...
`run_reserver()` uses `wait_metrics()` to wait until the node becomes the leader. Once it is, the task calls `refill()` and repeats every half-reservation interval. See [timestamp_oracle.rs:170-186][code-run-reserver]:
...
`refill()` first obtains a valid leader term. It then computes `now + width`, commits that upper bound through Raft, and stores the returned interval in `TimeService.reserved`. See [timestamp_oracle.rs:188-205][code-refill]:
...
![Three-node leader failover: Raft log replication and state-machine replay keep `reserved_end` consistent across nodes, so the new leader begins from a safe timestamp range](assets/time-server-failover-safety.png)
...
A lease is a time-bounded promise: as long as the leader has heard from a [quorum][post-quorum] recently, no other node can win an election. Checking that promise is a local read of heartbeat state, so it adds no network round trip to a timestamp request. `timestamp_oracle` performs that check with a [linearizable][post-linearizable] read using `ReadPolicy::LeaseRead`.
...
![Three-node timeline: heartbeat acknowledgments keep the old leader's lease valid; after the lease expires, a local check rejects `next_timestamp` requests, and the cluster then elects a new leader](assets/time-server-lease-handoff.png)
...
The HTTP handler, the request path, and the cache allocation fit in a few lines. See [timestamp_oracle.rs:124-133][code-take] and [timestamp_oracle.rs:162-168][code-next-timestamp]:
...
The term comparison in `take()` is not part of the lease check. The next section is why that field exists. `leader_term()` keeps the term and discards the log index; see [timestamp_oracle.rs:207-210][code-leader-term]. The [OpenRaft documentation][docs-openraft-readpolicy] describes the other `ReadPolicy` options. The underlying read optimization is covered in [How OpenRaft Optimizes ReadIndex][post-openraft-read].
...
![Cross-term timeline for three nodes: node A is the leader in terms 1 and 3, but its old reservation cache is invalidated; `next_timestamp` succeeds only after `refill` installs a new range for term 3](assets/time-server-reserved-lifecycle.png)
...
Run the command three times. Each request returns an [RFC 3339][rfc-3339] timestamp as a JSON string:
...
- Example: [timestamp_oracle.rs][repo-ezraft-timestamp-oracle]
...
- Previous article: [EzRaft: Build a Distributed KV Store in 100 Lines][post-ezraft-cn]
...
[code-reserved]: https://github.com/drmingdrmer/ezraft/blob/main/examples/timestamp_oracle.rs#L114-L148 "struct Reserved and its three methods"
...
[code-format]: https://github.com/drmingdrmer/ezraft/blob/main/examples/timestamp_oracle.rs#L225-L235 "format_timestamp and get_time"
[code-args]: https://github.com/drmingdrmer/ezraft/blob/main/examples/timestamp_oracle.rs#L237-L251 "command-line arguments"
