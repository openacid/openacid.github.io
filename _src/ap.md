---
title:      "统一分布式一致性: classic,flexible,multi-paxos/raft/2pc"
authors:
    - xp
categories:
    - algo
...
toc_label: 本文目录
toc_sticky: true
excerpt: "用一个算法描述classic-paxos, multi-paxos, raft, flexible-paxos, 2pc"
---

TODO: ec?

learn a theory , then forget it.


学习分布式协议时, 是否觉得 paxos 和 raft, 有点像又说不清哪里不像?
2pc 和 paxos 似乎都是两个阶段?
骚年, 你的感觉没有错, 所有这些分布式一致性算法, 都可以用一个统一的算法描述.

读完本篇, 手撕面试官不在话下, 本文帮你回答以下问题:

- 一致性的本质是什么?
- paxos 背后的隐藏技能
- raft 和 paxos 的区别?
- raft 和 multi paxos 怎么选择?
- raft 抢主活锁问题怎么优雅解决?
- 为何 raft 是一个糟糕的设计?
- multi paxos 如何支持乱序提交?
- 如何优化 raft/paxos 的读写?
- 2pc 和 paxos 的异同?
- 2pc 的 coordinator 失效问题怎么解决?
- 2pc upon raft 的分布式事务如何优化?
- 成员变更如何实现以及保证正确?
- 各种成员变更的算法优劣?
- 有没有更通用的成员变更策略?
- 组分裂和组合并

- TODO: 提出新的一致性算, 吸收所有的优点.
- TODO: paxos/raft的3副本如何优化?

- Paxos 的人话教程: [][post-paxos] [][post-paxoskv]
- Quorum 的人话教程: [][post-quorum]
- 成员变更的人话教程: [][post-raft-bug]
...
p1a, p1b, p2a, p2b 沿用 paxos 论文的术语, 分别表示第一阶段的请求/应答,
第二阶段的请求/应答.


## Quorum

majority() 函数用来返回一个replica集合的所有多数派, 例如
`majority({a, b, c}) = {ab, bc, ca}`.

Qw: 一个用来写的quorum, 是一个replia的集合, 例如`ab`.
Qr: 一个用来读的quorum, 是一个replia的集合
...
membership: 表示一个集群的成员配置, 这里用[][post-quorum] 中使用的 quorum 集合来表示,
例如一个三节点的 membership 为:`{ab, bc, ac}`.
这种方法的好处是不需要分别定义成员有哪些和 quorum 有哪些.
并且可以灵活的描述任何一种 quorum 设计. (raft的joint consensus描述神烦的)
...
Acceptor 的本地存储是一个二维结构, 以 Index 为key, 每个value是一组Cmd,
和一个指向上一个index的记录用于回退:
...
其中 `Index` 是任何一种满足 [][wiki-偏序关系] 的值类型, 例如整数或二维向量
Cmd是一个由用户定义的命令, 例如`set x=1` 或 `set i=i+1`.

由于 `Index` 本身可能是多维的值, 如一个向量,
所以 Acceptor 的结构也可以看做是一个n+1 维的空间, 空间中每个点是一个 `Cmd`.

例如以整数作为 `Index` 的类型, Acceptor 的存储结构如下:
...
Proposer 的数据结构类似, 也保存一个同样的 `Map<Index, {...}>`, 
```
...
算法本身的通讯协议类似于 paxos, 实际上是对paxos的推广
第一阶段 prepare 和 第二阶段的 accept.


- p1a: `P->A`:
  ```
  let all_indexes = P.keys()
  let upper = upper_bound(all_indexes)
  let next = next(upper)

  send({ index: next })
  ```

  P 找到下一个大于所有已知 index 的 index, 发给一个读quorum: Qrᵢ.
  同样, 2个 P 生成的 `index` 是不允许相同的.

- p1b: `A->P`: 如果请求中的 `p1a.index` 大于所有 Acceptor 本地知道的index,
  则在本地初始化一个新的 map entry:
  ```
  A[p1a.index] = {
    prev_index: A.last_index()
  }
  ```
  否则什么也不做.
...
- 如果 P 收到的应答中, `p1b.last_index()` 跟发送的一样, 则代表成功,
    类似 paxos 的 preapre 一个 ballot-num.

  如果 P 收到了某个Qrᵢ 的所有成功应答, 则第一阶段完成.
  后面可以多次运行第二阶段提交数据.

  找出所有应答中最大 index 的 State, 合并
  如果membership跟P.membership不同, 替换.


- p2a: `P->A`: 持续增加新的cmd并发送给Acceptors
     收到来自一个 Qw 的应答表示已提交.
...
cmd_mem 包含下一个要使用的 quorum 集合
例如3节点的所有quorum的集合是: {ab, ac, bc}

cmd_mem提交到自己Qw中
因此要保证cmd_mem与所有正在使用quorum有交集.
...
但是{xy,ab,cd} 不可以, 如果立即使用xy作为一个Qw,
提交到xy 这2个节点的信息可以直接继续运行读写,
而这时可能{ab,bc} 没能提交, 被其他只覆盖.


最新 committed membership 之前的 membership 不需要考虑.
因为 cmd_mem 与所有其他 Cmd 相关, 所以 cmd_mem 之间有全序关系.


# classic-paxos

Index : (n, proposer_id), 字典序
State : [] or [cmd]

static membership: P.v.membership = A.v.membership:

Q.w = majority(acceptors)
Q.r = majority(acceptors)
...
Q.W = {r[i*m+k]} for some i
Q.R = {r[i*m+k]} for some k

```
Acceptors:

A11 A12 A13  <-- membership.w
A21 A22 A23
A31 A32 A33
...
Index : (term, id) 大小关系: Index₁ > Index₂ => term₁ > term₂
State : [cmd₁, cmd₂ ...],  cmd 不能是nil, cmd 可以是membership config


dynaimc membership: P.V.Q = A.V.Q:

Qw = Qr = latest membership
...
- raft中, 因为State是连续日志, 所以不需要合并, 合并就是选择最大的V.
  于是一个优化是去掉合并V的过程, 让较小的V直接放弃P的角色(也就是选主的角色).
  所以在选举过程中(第1, 2阶段), 自己的V如果较小就放弃.

- raft 不支持连续的成员变更, 即上一个变更未提交之前不允许提交下一次,
  并且引入了2阶段:

  当上一个变更提交后才允许新的变更, 所以只需要2个配置的笛卡尔积, 也就是joint
  config.

  joint 提交后, 用第2条日志变更为普通配置.

  TODO

- Index 不保存, 通过日志拿. TODO
    我们没有定义State中如何去唯一确定一个元素,
    这算是应用层的一个问题.
    因为在分布式系统中, 不同的P试图提交的完全相同的一个变更也应视为不同的.
    例如, 在 classic-paxos中, 2个proposer同时提交了cmd:x=x+1的命令,
    那么最终应该有2个x=x+1命令被执行而不是一个.

    因此, 一般在分布式系统中, 都会在cmd之外额外添加一个 proposer的id,
    以区分不同proposer 试图提交的相同的命令.

    在apaxos中, 可以将每个cmd都加上一个Index来实现这个目的.

    几乎所有的分布式系统都是这么做的.
    raft也一样.

    raft的cmd的内容为: (Index, cmd)
    但是因为raft中, Index定义为(term, id)
    而同一个term只有一个合法的leader.
    所以它可以省去id的部分:
    cmd: (term, cmd)

    ---

    我们也知道A 存储是: (NextIndex, V=(Index, Q, State))
    (term, id), V=((term, id), logs=[(term, cmd), (term, cmd)...])

    因为一个term只有一个leader, 所以:
    (term, id), V=(term, logs=[(term, cmd), (term, cmd)...])
...
- 同时也能很容易看出raft设计上的一个问题: 只用了term比较Index大小

membership: majority(Cᵢ) or majority(Cᵢ) X majority(Cᵢ₊₁)
...
Index : BallotNum: {id:1}, vector order
idx : VBal:
ud  : [] or [cmdᵢ]:
...
# A consensus with advantages from all 

pre : BallotNum: (last_term+1, last_log_index, id:1) dict order

ud  : {[cmd₁, cmd₂ ...], [membership₁, membership₂...]}
