...
p1 写入 ud1
如果允许覆盖, 
那么p0 写入 到qw的数据就可能被后面的p1覆盖掉.
也就是说无法保证写入就被读到.

任何一次写重试都可能造成状态丢失

要想达到状态不丢失, 就必须保证任意2个值v1和v2, 最多只能单向被覆盖
也就是说, 写入的数据至少是 partial order的.
因为读取时, 如果看到2个不同的v1 v2, 必须只能选一个, 于是
v1 v2 必须是 total order的

因为ud是任意的, 即使是total order的, ud本身的顺序也不代表
一致性的顺序.
所以写入的v必须由写入者包装成total order的: 即v = (idx, ud)

其中idx是一个 total order, 
顺序定义为: v1 > v2 => idx1 > idx2
...
现在系统是一个典型的quorum rw系统:
状态单调变化, 不可回退

#

多个proposer 之间分配的idx不同, 
要保证一致性, 已经写入qw的值不能被覆盖.
例如5节点中, 
qw=qr=3, 
写入了3个(1, ud1)后, proposer认为已经写入成功了.
那么后续的读取应该能读到这个状态, 
也就是说不允许(2, ud2) 也写入成功.

于是proposer必须保证没有比它小的v能写入成功, 
再开始自己的写入

因为写入是单调的,
因此p2要写入(2, ud2)之前, 必须把一个信息: `idx<2`的写入都不允许成功,
这个节点集合跟所有其他qw都有交集, 
这样就可以保证p2写入qw2后, 没有更小的v被写入.

因此写入的值还需要引入另一维度: v := (pre, [(idx, ud), (idx, ud)...])
且, 后续写入的idx == pre, 以保证它的fence作用生效

v1 > v2 => v1.pre > v2.pre || v1.pre == v2.pre && v1.idx > v2.idx
...
pre : BallotNum: (n, proposer_id)
idx : VBal:
ud  : any value: partial eq relation:
